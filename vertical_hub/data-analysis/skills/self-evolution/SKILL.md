---
name: self-evolution
description: 自进化机制 — verifier cron 在分析报告连续不通过时自动纠偏。按 evolution_policy.md 策略执行。
available: true
---
...
自进化 = verifier cron 自动发现分析报告的质量问题 → 定位原因 → 修改规则 → 验证效果。
...
1. **收集上下文** — `quality_signal list` 读取最近检查结果，理解哪些报告在哪些维度失败
...
| 文件 | 路径 | 说明 | 操作工具 |
|------|------|------|----------|
| 任务列表 | `tasks.md` | 所有分析任务和状态 | `read_file` |
| 分析报告 | `reports/phase{N}/` | executor 产出的报告 | `read_file` |
...
| 对齐规则 | `alignment.md` | 分析标准和约束 | `read_file` |
...
| `quality_check` | 无写入，返回检查结果 | 按维度检查报告 |

### Skill 文件位置（通过工具间接访问）

| 文件 | 访问方式 | 说明 |
|------|----------|------|
| `table-analysis/SKILL.md` | `skill_view("table-analysis")` 加载后用 `edit_file` 修改 | 单表分析技能 |
| `cross-table-analysis/SKILL.md` | `skill_view("cross-table-analysis")` 加载后用 `edit_file` 修改 | 跨表关联技能 |
| 其他 skill | `skill_view("{skill_name}")` | 按需加载 |
...
示例：
```
- [对比分析] 当任务涉及时间维度时，必须包含环比对比数据
- [异常检测] 当金额偏离均值 2σ 以上时，必须标记为异常并说明原因
- [企业分析] 当 TOP 企业占比 >30% 时，必须单独分析该企业
```
...
示例：
```
问题：关联分析 skill 没有要求标注数据来源
→ skill_view("cross-table-analysis")
→ edit_file 追加步骤"标注每个结论的数据来源表和查询条件"
```
...
```
问题：缺少跨表关联分析能力
→ 新增 cross_table_analysis workflow
```
