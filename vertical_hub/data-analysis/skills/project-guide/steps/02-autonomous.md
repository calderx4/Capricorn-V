...
> 用 `cron create` 创建 role=executor 的定时任务，自动从 tasks.md 领取分析任务执行。
...
> 注：verifier 角色的完整指令在角色模板中，包括：扫描已完成报告 → quality_check → quality_signal record → 连续不通过时 bia_update 纠偏 → changelog 记录。prompt 只需写简短触发语。

3. **验证运行** — 确认能正确读取报告并输出质量检查结果
...
executor cron（每小时）        verifier cron（每天或每4小时）
    ...
    ├─ 执行分析，写报告             ├─ quality_signal record
...
| 产出 | 位置 | 说明 |
|------|------|------|
| 分析报告 | `reports/phase{N}/` | executor 每次执行产出 |
